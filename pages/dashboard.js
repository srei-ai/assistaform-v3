...
export default function Dashboard() {
  ...
  const onCsvFile = async (e) => {
    ...
    const text = await file.text(); const headers = parseCsvHeader(text)
  ...
  const createForm = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/forms', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ title, fields }) })
      const data = await r.json(); if (!r.ok) throw new Error(data.error || 'Failed to create form')
      setCreated(data)
  ...
  return (
    <main>
      <div className="card" style={{padding:20}}>
        ...
        {error && <div className="badge" role="status" aria-live="polite" style={{marginTop:8, background:'#ffecec', borderColor:'#ffd2d2', color:'#5f2120'}}>⚠️ {error}</div>}
        <div style={{ marginTop: 12 }}>
          <button onClick={createForm} disabled={busy} className="btn btn-primary">{busy ? 'Creating…' : 'Create form'}</button>
        </div>
        {created && (
          <section style={{marginTop:20}}>
            <hr className="sep" />
            <p>Form created <span className="badge">✅ Saved</span></p>
            <div className="row"><code>id: {created.id}</code>{created.public_id && <code>public_id: {created.public_id}</code>}</div>
            <div className="row" style={{marginTop:8}}>
              ...
              <a href={`/analytics/${shareId}`} className="btn">View analytics</a>
            </div>
